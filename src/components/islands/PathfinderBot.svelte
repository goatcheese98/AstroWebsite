<script lang="ts">
  ...
  type ScreenMessage = "smile" | "hi" | "stars" | "zzz" | "tetris";
  type ClickAnimation =
    | "right-arm"
    | "left-arm"
    | "left-leg"
  ...
  function openTetrisMenu(event?: Event) {
    stopEvent(event);
    idleTime = 0;
    if (robotState === "sleeping") {
      robotState = "idle";
    }
    screenMessage = "tetris";
    if (tetrisMode === "closed") {
      tetrisMode = "menu";
    }
  }

  function closeTetris(event?: Event) {
    stopEvent(event);
    tetrisMode = "closed";
  ...
  function scheduleWave() {
    ...
    waveTimeout = setTimeout(() => {
      if (robotState === "idle" && tetrisMode === "closed") {
        robotState = "waving";
        screenMessage = "hi";
        setTimeout(() => {
          robotState = "idle";
  ...
  $effect(() => {
    ...
    idleTimer = setInterval(() => {
      idleTime++;
      if (idleTime > 300 && robotState === "idle" && tetrisMode === "closed") {
        robotState = "sleeping";
        screenMessage = "zzz";
      }
    }, 50);
  ...
  const isTetrisOpen = $derived(tetrisMode !== "closed");
  ...
  const screenBg = $derived(
    screenMessage === "tetris" || isTetrisOpen
        ? "#0a0a14"
        : isExcited || isLove
          ? "#fca5a5"
          : isHappy
...
<div bind:this={containerRef} class="container">
  <div
    ...
    tabindex="0"
    onclick={handleClick}
    onkeydown={(e) => e.key === "Enter" && tetrisMode === "closed" && handleClick()}
      ...
      >
        ...
        <g clip-path="url(#screenClip)" style="transform-origin: 150px 220px;">
          ...
          {:else if screenMessage === "tetris"}
            ...
            {#if tetrisMode === "menu"}
              <text x="150" y="206" text-anchor="middle" fill="#e2e8f0" font-size="7.5" font-family="monospace">
                READY TO PLAY?
              </text>
              ...
              <g class="tetris-ui-button tetris-ui-secondary" onclick={(event) => closeTetris(event)}>
            ...
            {:else}
              {#each tetrisRenderGrid as row, rowIndex}
                {#each row as cell, colIndex}
                  <rect
                    ...
                    rx="0.8"
                    fill={cell === 0 ? "rgba(148, 163, 184, 0.12)" : TETRIS_COLOR_BY_ID[cell]}
                  />
                {/each}
              {/each}

              <g class="tetris-control-button" onclick={(event) => handleTetrisAction("pause", event)}>
                <rect x="167" y="194" width="10" height="8" rx="2" fill="#334155" />
                <text x="172" y="199.5" text-anchor="middle" fill="#e2e8f0" font-size="4.6" font-family="monospace">
                  P
                </text>
              </g>
              <g class="tetris-control-button" onclick={(event) => closeTetris(event)}>
                <rect x="180" y="194" width="10" height="8" rx="2" fill="#334155" />
                <text x="185" y="199.5" text-anchor="middle" fill="#e2e8f0" font-size="4.6" font-family="monospace">
                  X
                </text>
              </g>
              ...
              <rect x="167" y="208" width="23" height="21" rx="2.5" fill="#0f172a" stroke="#334155" stroke-width="0.7" />
              ...
              {:else}
                <g class="tetris-control-button" onclick={(event) => handleTetrisAction("rotate", event)}>
                  <rect x="172" y="234" width="12" height="9" rx="2" fill="#1e293b" />
                  <text x="178" y="240" text-anchor="middle" fill="#e2e8f0" font-size="5.5" font-family="monospace">
                    ^
                  </text>
                </g>
          ...
          {#if screenMessage !== "tetris" && tetrisMode === "closed"}
            <g class="tetris-ui-button" onclick={(event) => openTetrisMenu(event)}>
        ...
        >
          ...
          <circle
            ...
            stroke-width="1"
          />
          <!-- Antenna ball + orbital ring group -->
...
<style>
  ...
  /* Idle breathing for the whole bot */
  ...
  @keyframes shadow-shrink {
    ...
    40%, 70% { transform: scaleX(0.45); opacity: 0.03; }
  }
</style>
