...
const INITIAL_CATEGORIES: SkillCategory[] = [
  {
    id: 'data',
    name: 'Data & Analytics',
    icon: '📊',
...
export default function BentoSkills() {
  const [categories, setCategories] = useState<SkillCategory[]>(INITIAL_CATEGORIES);
  ...
  const toggleExpand = useCallback((id: string) => {
    setCategories((prev) =>
      prev.map((cat) => ({
        ...cat,
        expanded: cat.id === id ? !cat.expanded : cat.expanded,
      }))
    );
  }, []);
  ...
  if (!isClient) {
    return (
      <div className="bento-skills skeleton">
    ...
    <div className="bento-skills">
      <div className="bento-grid">
        {categories.map((category) => (
          <div
            key={category.id}
            className={`bento-card ${category.expanded ? 'expanded' : ''}`}
            style={{
          ...
          >
            <div className="bento-card__inner">
              <div className="bento-card__header">
                <span className="bento-card__icon">{category.icon}</span>
                <div className="bento-card__title-group">
                  <h3 className="bento-card__name">{category.name}</h3>
                  <p className="bento-card__desc">{category.description}</p>
                </div>
                <span className="bento-card__expand-hint">
                  {category.expanded ? '−' : '+'}
                </span>
              </div>
              
              <div className={`bento-card__skills ${category.expanded ? 'expanded' : ''}`}>
                {(category.expanded ? category.skills : category.skills.slice(0, 3)).map((skill) => (
                  <span
                    key={skill.id}
                    className="skill-tag"
                  >
